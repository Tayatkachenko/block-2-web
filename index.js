...
document.addEventListener('DOMContentLoaded', function(){
   ...
   document.getElementById('filterByAgeButton').addEventListener('click', function() {
       ...
       displayResults(filteredByAge, 'Результати за фільтром "За віком 18 років":');
   });
   ...
   if (filteredData.length > 0) {
       filteredData.forEach(item => {
           ...
           resultItem.innerHTML = `
               <div><strong>Як Вам наші послуги?</strong> ${item.q1}</div>
               <div><strong>Вкажіть вашу дату народження:</strong> ${item.q2}</div>
               <div><strong>Скільки країн Ви відвідали?</strong> ${item.q3}</div>
               <div><strong>Напишіть Ваші зауваження до обслуговування</strong> ${item.q4}</div>
